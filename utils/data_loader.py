...
class ConvECorpus(Corpus):
    ...
    # def check_multi_label(self):
    #     total_samples = len(self.train_indices)
    #     multi_label_count = 0

    #     for sample in self.train_indices:
    #         labels = sample['label']  # 取出 label 列表
    #         if len(labels) > 1:
    #             multi_label_count += 1

    #     print(f"总样本数量: {total_samples}")
    #     print(f"存在多标签的样本数量: {multi_label_count}")
    #     print(f"多标签占比: {multi_label_count / total_samples * 100:.2f}%")

    #     if multi_label_count > 0:
    #         print(" 你的数据集中确实存在多标签样本")
    #     else:
    #         print(" 你的数据集实际上是单标签的")
    ...
    def get_batch_new(self, batch_num):
        if (batch_num + 1) * self.batch_size <= len(self.train_indices):
            batch = self.train_indices[batch_num * self.batch_size: (batch_num+1) * self.batch_size]
        else:
            batch = self.train_indices[batch_num * self.batch_size:]
    ...
    def get_validation_pred(self, model, split='test'):
        ...
        for batch_num in range(max_batch_num):
            ...
            pred, _ = model.forward(head_batch_indices)
            ...
            # pred = pred[0] * entropy_weights[0] +pred[1] * entropy_weights[1]+ pred[2] * entropy_weights[2]+ pred[3] * entropy_weights[3]
            ...
            pred, _ = model.forward(tail_batch_indices)
            for i in range(pred[0].shape[0]):
