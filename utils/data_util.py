...
def load_data(datasets):
    ...
    # Try loading CLIP features first, then fallback to standard features
    if os.path.exists(path+'img_features_clip.pth'):
        print(f"Loading CLIP image features from {path}img_features_clip.pth")
        img_features = torch.load(open(path+'img_features_clip.pth', 'rb'))
    elif os.path.exists(path+'img_features.pth'):
        print(f"Loading standard image features from {path}img_features.pth")
        img_features = torch.load(open(path+'img_features.pth', 'rb'))
    else:
        raise FileNotFoundError(f"Could not find image features in {path}")

    if os.path.exists(path+'text_features_clip.pth'):
        print(f"Loading CLIP text features from {path}text_features_clip.pth")
        text_features = torch.load(open(path+'text_features_clip.pth', 'rb'))
    elif os.path.exists(path+'text_features.pth'):
        print(f"Loading standard text features from {path}text_features.pth")
        text_features = torch.load(open(path+'text_features.pth', 'rb'))
    else:
        raise FileNotFoundError(f"Could not find text features in {path}")
...
def load_more_data(datasets, numeric=True):
    ...
    text_features = torch.load(open(path+'text_features.pth', 'rb'))
    if numeric:
        num_features = torch.load(open(path+'numeric_features.pth', 'rb'))
    else:
        num_features = None
    print(num_features)
    return entity2id, relation2id, img_features, text_features, num_features \
           (train_triples, train_adj, train_unique_entities), \
           (val_triples, val_adj, val_unique_entities), \
           (test_triples, test_adj, test_unique_entities)
